...
namespace Game.Astronomy.Core
{
    public static class AstronomyCore
    {
        ...
        private static void RenewSpaceObject(string spaceobject_id)
        {
            ...
            {
                float ecos = 1 - SO.relative_movement.eccentr * (float) Math.Cos(SO.relative_movement.ell_angle);
                float r = (ecos / (1 + SO.relative_movement.eccentr)) * SO.scaling_properties.self_scale *
                          SP.result.total_size;
                SO.result.system_angle = SP.result.system_angle + SO.relative_movement.ell_angle +
                                         SO.relative_movement.orb_angle;
